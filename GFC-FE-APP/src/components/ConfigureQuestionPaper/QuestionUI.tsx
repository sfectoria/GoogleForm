...
export function QuestionForm() {
  ...
  const updateDocument = (): void => {
    handlePromiseRequest(async () => {
      try {
        console.log("Questions before deduplication:", questions);
        const deduplicatedQuestions = deduplicateQuestions(questions);
        console.log("Questions after deduplication:", deduplicatedQuestions);
        ...
        const response = await HttpRequestController(REQUEST_URLS.UPDATE_DOCUMENT, HTTP_METHODS.PUT, payload);
        ...
        toast.error(REQUEST_FAILURE_MESSAGES.SAVING_QUESTIONS_FAILED);
      }
 ...
 // Function to deduplicate questions
...
// Generate a unique key for each question based on its text and options
const generateQuestionKey = (question: Question): string => {
  const optionsString = question.options
    .map(option => option.option) // Ensure `option.option` is correct
    .sort() // Sort options to ensure consistent key
    .join(',');
  
  return `${question.question}-${optionsString}`;
};

console.log(answers,"answ");
const handleValueChange = (question: Question, value: string, option?: string, checked?: boolean) => {
  setAnswers((prevAnswers) => {
    // Handling non-checkbox input types
    if (question.questionType !== QUESTION_TYPES.CHECKBOX) {
      return { ...prevAnswers, [question._id]: value };
    } else {
      // Handling checkbox input type
      const selectedOptions = prevAnswers?.[question._id] ?? [];
      if (checked) {
        // Add the option if it's checked
        return { ...prevAnswers, [question._id]: [...selectedOptions, option] };
      } else {
        // Remove the option if it's unchecked
        const updatedOptions = selectedOptions.filter((selected: string) => selected !== option);
        return { ...prevAnswers, [question._id]: updatedOptions };
      }
    }
  });
};
  ...
  const saveUserResponse = async (): Promise<void> => {
    if (checkAllRequiredQuestionsAreAnswered()) {
      ...
      await HttpRequestController(`${REQUEST_URLS.USER_RESPONSE}`, HTTP_METHODS.POST, payload);
